...
According to this analysis, there are 8,540 low-income tracts and 245 contiguous income tracts. These numbers are 
...
(* *i.e pennisula tracts*). When analysis on a per state level, the number of Opportunity Zones (OZs) per state matches the CEA report
for all inland states but there are several additional tracts listed as OZs in Guam that have the same census tract number.
...
is a presentation of the differences in designated and non-designated OZ census tracts. This analysis finds that the average 
poverty rate for census tracts designated as OZs are a little below double that of all other census tracts and have higher shares
...
that are considered to be more impoverished communities compared to others in the United States. 
...
to conduct futher analysis to determine if this holds true on a state level. Below is a figure that compares the 
...
eligible OZs (not designated), and non-eligible census tracts for each state. This can be used to verify that so far the designation
process is somewhat efficient as the designated OZs on average align with the purpose of the program.
...
Below are some additional economic metrics related to unemployment. Each graph compares the Unemployment Rates
and Labor Force Participation Rates of designated OZs, eligible OZs (not designated) and the United States (as a whole).
Figure 4 and Figure 5 both shows that on average, designated OZs have a higher unemployment rates and lower 
labor force particpation compared to the United States' average. Hopefully with OZs being designated in such areas, 
this will lead to job creation and give these areas a much needed economic boost that has been long overdue. 
...
(*comparable to FW-Dodge data*). This particular data set goes back to 2000 and has information on construction and 
...
I have not been able to access this data yet but I will be getting a small snippet of it (3 states) to see what I can do with it
but judging from the conservation it seems promising. 
